...
GoogleDrive.prototype = {
  ...
  init: function () {
    var that = this;
    return new Promise(function (resolve, reject) {
      that.readFile(SECRET_PATH).then(function (secret) {
        return that.createAuth(secret);
      }, function(err){
        console.log(err);
      }).then(function (auth) {
        that.auth = auth;
  ...
  createAuth: function (secret) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      try {
        ...
        var redirectUrl = secret.installed.redirect_uris[0];
  ...
  getTokenUrl: function (redirectUrl) {
    ...
    var ret = this.oauth2Client.generateAuthUrl({
      access_type: 'offline',
      scope: SCOPES,
      redirect_uri: url
    });
